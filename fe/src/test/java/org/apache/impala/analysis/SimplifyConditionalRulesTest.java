...
public class SimplifyConditionalRulesTest extends BaseRewriteRulesTest {
  ...
  public void testCaseWithoutExpr() throws ImpalaException {
    ...
    // A TRUE condition short-circuits all remaining expressions.
    // TODO: IMPALA-xxxx
    RewritesOk(
        "case when id = 0 then 0 when true then 1 when id = 2 then 2 else 3 end",
        rule, null);
    // Want: case when id = 0 then 0 else 1 end
  }
  ...
  @Test
  public void testRewriteDecode() throws ImpalaException {
    RewritesOk("decode(id, 1, 10)", rule,
        "CASE WHEN id = 1 THEN 10 END");
    RewritesOk("decode(id, int_col, 10)", rule,
        "CASE WHEN id IS NOT DISTINCT FROM int_col THEN 10 END");
    RewritesOk("decode(id, null, 10)", rule,
        "CASE WHEN id IS NULL THEN 10 END");
    RewritesOk("decode(id, 1, 10, 2, 20)", rule,
